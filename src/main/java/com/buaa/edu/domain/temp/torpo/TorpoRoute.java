...
import com.buaa.edu.domain.temp.visual.VisualTorpo;
import jxl.Sheet;
...
public class TorpoRoute {
    ...
    public int longRoute;
    public int longLevels;
    public int max_lines;

    private ArrayList<Pair<Integer, Integer>> netEleLinks;
    private ArrayList<Integer> netEles;
    ...
    public TorpoRoute() {
        ...
        this.netEles = new ArrayList<>();
        this.netEleLinks = new ArrayList<>();
    }
    ...
    public void flushLevel(int route_type, int level, int line) {
        if (level > longLevels) {
            longLevels = level;
            this.longRoute = route_type;
        }
        if (line > max_lines) {
            max_lines = line;
        }
    ...
    public HashMap<String, TorpoDevice> getDevice_map() {
        return device_map;
    }
    ...
    private boolean LinkAndEraseDevs(TorpoDevice in_dev, TorpoDevice out_dev, int route_type) {
        ...
        int netEle1 = in_dev.line_id;
        int netEle2 = out_dev.line_id;

        if (!this.netEles.contains(netEle1)) {
            this.netEles.add(netEle1);
        }
        if (!this.netEles.contains(netEle2)) {
            this.netEles.add(netEle2);
        }

        Pair<Integer, Integer> pair = new Pair<>(netEle1, netEle2);

        if (!this.netEleLinks.contains(pair)) {
            this.netEleLinks.add(pair);
        }
    ...
    private void UpdateTorpoData(int route_type) {
        ...
        for (String str:
            ...
            dev.FlushData(0, route_type, line);
            line++;
    ...
    public ArrayList<Integer> getNetEles() {
        return this.netEles;
    }

    public ArrayList<Pair<Integer, Integer>> getNetEleLinks() {
        return netEleLinks;
    }
}
